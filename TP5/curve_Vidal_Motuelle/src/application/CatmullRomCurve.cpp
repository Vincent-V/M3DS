...
void CatmullRomCurve::setup() {
  ...
  if(nbPoint()>3){
      for(int i=1;i<nbPoint()-1;i++){
          ...
          intermediate(i-1,1,point(i)-parallele);
          intermediate(i,0,point(i)+parallele);
      }
      intermediate(0,0,(point(0)+(intermediate(0,1)-point(0))/4));
      intermediate(nbPoint()-2,1,  (point(nbPoint()-1)+((intermediate(nbPoint()-2,0)-point(nbPoint()-1))/4))  );
  }
