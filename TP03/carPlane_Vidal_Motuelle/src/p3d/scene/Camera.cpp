...
Vector2 Camera::windowToNDC(const Vector2 &u) {
  double xWindow=(double(u.x()-_viewport[0])/_viewport[2])*2-1;
  double yWindow=(double(u.y()-_viewport[1])/_viewport[3])*2-1;
  return Vector2(xWindow,yWindow);
}

bool Camera::windowToCamera(const Vector2 &u,Vector3 *mouseCam) {
  Vector2 ndc=windowToNDC(u);
...
bool Camera::windowToCamera(const Vector2 &pointWindow,const Vector3 &pointPlane,Vector3 *pointCam) {
  Vector2 ndc=windowToNDC(pointWindow);
  if ((ndc.x()<-1) || (ndc.x()>1) || (ndc.y()<-1) || (ndc.y()>1)) {
    return false;
  }
  else {
...
bool Camera::windowToWorld(const Vector2 &pointWindow,const Vector3 &pointPlane,Vector3 *pointCam) {
  if (windowToCamera(pointWindow,pointPlane,pointCam)) {
    pointTo(Coordinate_Parent,pointCam);
    return true;
  }
  else return false;
}
...
bool Camera::windowToRayWorld(const Vector2 &mouse,Line *l) {
  ...
  if (!ok) return false;
    ...
    pointTo(Coordinate_Parent,&a);
    directionTo(Coordinate_Parent,&u);
    l->set(a,u);
    return true;
  }
}
