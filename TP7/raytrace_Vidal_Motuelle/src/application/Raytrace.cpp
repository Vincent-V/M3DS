    ...
    if (level>0 && contribution>0.001) {
        ...
        if (inter!=NULL) { // existe-t-il une intersection avec la scène ?
            ...
            if (coeffReflexion > 0.0 && level > 0 && contribution > 0.001) {
                ...
                Vector3 reflectedRayColor = computeRayColor(reflectedRay,level-1,contribution*coeffReflexion);
                Vector3 refractRayColor = computeRayColor(refractRay,level-1,contribution*coeffReflexion);
